...
export type MediaFolder =
  "baby" | "album" | "prescriptions" | "parents" | "documents" | "life-events";
...
export async function uploadMedia(
  ...
  if (!familyId)
    throw new MediaError("Sign in first so photos can be saved to your family account.");
  if (!file.type.startsWith("image/"))
    throw new MediaError("Please choose an image (JPG, PNG or WebP).");
...
export async function uploadDocument(familyId: string | null, file: File): Promise<string> {
  if (!familyId)
    throw new MediaError("Sign in first so this document can be saved to your family account.");
  ...
  const rawExt =
    file.name.split(".").pop()?.toLowerCase() ?? (file.type === "application/pdf" ? "pdf" : "jpg");
