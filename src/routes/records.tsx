  ...
  return (
    <AppShell>
      <PageHeader
        ...
        subtitle={`${store.baby.name}'s story, health and documents`}
      />
      <div className="space-y-4 px-5 pb-6">
        <SoftCard className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold">A record that grows with your child</p>
              <p className="text-xs text-muted-foreground">
                Existing trackers remain the source of truth.
              </p>
            </div>
          </div>
        </SoftCard>
        ...
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <TabsContent value="event">
            <SoftCard className="space-y-3">
              <select
                value={category}
                onChange={(e) => setCategory(e.target.value as LifetimeCategory)}
          ...
          <TabsContent value="documents" className="space-y-3">
            <SoftCard className="space-y-3">
              <select
                value={docCategory}
                onChange={(e) => setDocCategory(e.target.value as MedicalDocumentCategory)}
                className="h-11 w-full rounded-2xl border bg-background px-3 text-sm"
              >
              ...
              <input
                ...
                className="hidden"
                onChange={(e) => {
                  void addDocument(e.target.files?.[0]);
                  e.target.value = "";
                }}
              />
              <p className="text-[11px] text-muted-foreground">
                Images and PDFs are protected by your family account.
              </p>
            </SoftCard>
            {store.medicalDocuments.length === 0 ? (
              <SoftCard className="text-center text-sm text-muted-foreground">
                No medical documents saved.
              </SoftCard>
            ) : (
              store.medicalDocuments.map((document) => (
                <SoftCard key={document.id} className="flex items-center gap-3 p-4">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void mediaUrl(document.objectPath).then(
                        (url) => url && window.open(url, "_blank", "noopener,noreferrer"),
                      )
                    }
                  >
                    <p className="truncate font-bold">{document.title}</p>
                    <p className="text-xs capitalize text-muted-foreground">
                      {document.category.replaceAll("_", " ")} · {formatDate(document.documentAt)}
                    </p>
                  </button>
                  <button
                    ...
                    onClick={() => {
                      ...
                      store.deleteMedicalDocument(document.id);
                      void removeMedia(document.objectPath);
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          <Link to="/track/weight" className="rounded-2xl bg-secondary p-3 text-center font-bold">
            Growth tracker
          </Link>
          <Link to="/track/doctor" className="rounded-2xl bg-secondary p-3 text-center font-bold">
            Doctor visits
          </Link>
          <Link to="/track/album" className="rounded-2xl bg-secondary p-3 text-center font-bold">
            Memories
          </Link>
          <Link
            to="/timeline"
            search={{ days: 0, type: "all" }}
            className="rounded-2xl bg-secondary p-3 text-center font-bold"
          >
            Daily timeline
          </Link>
        </div>
